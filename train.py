...
tf.flags.DEFINE_integer("num_epochs", 50, "Number of training epochs (default: 200)") # default: 50
tf.flags.DEFINE_integer("evaluate_every", 100, "Evaluate model on dev set after this many steps (default: 100)")
tf.flags.DEFINE_integer("checkpoint_every", 300, "Save model after this many steps (default: 100)") # default: 1000
# Misc Parameters
...
print("Loading data...")
n_dev_samples = 2000 
x, y = data_helper.load_data(4000, 4000)
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def dev_step(x_batch, y_batch, writer=None):
            ...
            f = open('devs/devs-2_2_model-' + str(FLAGS.dropout_keep_prob)  \
                    + '-' + str(FLAGS.l2_reg_lambda), 'a')
            for i in range(num_batches):
                x_batch_dev, y_batch_dev = data_helper.get_shaped_batch_input(
            ...
            if len(acc) != 0:
                ...
                f.write("\nMean accuracy=" + str(sum(acc)/len(acc)) + '\n')
                f.write("Mean loss=" + str(sum(losses)/len(losses)) + '\n')
            f.close()

        # Generate batches in shaped batch format
        batches = data_helper.batch_iter(x_train, y_train, FLAGS.batch_size, FLAGS.num_epochs)
