...
class CharCNN(object):
    ...
    def __init__(self, num_classes=2, filter_sizes=(7, 7, 3, 3, 3, 3), num_filters_per_size=256,
                 l2_reg_lambda=0.0, sequence_max_length=20000, num_quantized_chars=8):
        ...
        with tf.name_scope("conv-maxpool-6"):
            ...
            W = tf.Variable(tf.truncated_normal(filter_shape, stddev=0.05), name="W")
            b = tf.Variable(tf.constant(0.1, shape=[num_filters_per_size]), name="b")
            conv = tf.nn.conv2d(pooled, W, strides=[1, 1, 1, 1], padding="VALID", name="conv6")
            h = tf.nn.relu(tf.nn.bias_add(conv, b), name="relu")
            pooled = tf.nn.max_pool(
                h,
                ksize=[1, 1, 3, 1],
        ...
        # ================ Layer 7 ================
        num_features_total = pooled.get_shape().as_list()[2] * num_filters_per_size
        ...
        with tf.name_scope("fc-3"):
            ...
            self.scores = tf.nn.xw_plus_b(fc_1_output, W, b, name="output")
